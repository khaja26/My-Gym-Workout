...
document.addEventListener("DOMContentLoaded", () => {
  ...
  table.addEventListener("click", (event) => {
    ...
    // Skip if no row or it's the header
  ...
  completedButton.addEventListener("click", () => {
    ...
    const [date, day, activity, hours, minutes] = rowData;

    // Save the completed status to the database
    saveToDatabase({ date, day, activity, hours, minutes, status: "completed" });
  });

  // Save data to the database
  function saveToDatabase(rowData) {
    const { date, day, activity, hours, minutes, status } = rowData;

    // Example SQLite logic using SQL.js
    const dbRequest = new XMLHttpRequest();
    dbRequest.open("POST", "/save", true);
    ...
    dbRequest.onreadystatechange = () => {
      if (dbRequest.readyState === 4 && dbRequest.status === 200) {
        console.log("Data saved successfully:", dbRequest.responseText);
      }
    };

    const payload = JSON.stringify({ date, day, activity, hours, minutes, status });
    dbRequest.send(payload);
  }
});
